  <?php $this->extends = "auth.main";?>
  <?php ob_start(); $this->currentSection = "main-content";?>
			<div class="container">
				<div class="card py-5">
					<div class="row g-0">
						<div class="col col-xl-5">
							<div class="card-body p-4">
								<h1 class="display-1"><span class="text-primary">4</span><span class="text-danger">0</span><span class="text-success">4</span></h1>
								<h2 class="font-weight-bold display-4">Lost in Space</h2>
								<p>You have reached the edge of the universe.
									<br>The page you requested could not be found.
									<br>Dont'worry and return to the previous page.</p>
								<div class="mt-5">
									<a href="<?php echo esc(BASE_URL);?>/home" class="btn btn-outline-dark btn-lg ms-3 px-md-5 radius-30">Back</a>
								</div>
							</div>
						</div>
						<div class="col-xl-7">
							<img src="assets/images/errors-images/404.png" class="img-fluid radius-10" alt="" 
							style="padding: 10px;
									 max-height:500px; 
									 margin:auto;
									 border: 1px, solid; 
									 border-radius:10px">
						</div>
					</div>
					<!--end row-->
				</div>
			</div>

  <?php $this->sections["$this->currentSection"] = ob_get_clean();