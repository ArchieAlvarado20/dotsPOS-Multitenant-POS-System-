...
class Validator extends Database
{
	...
	protected function validate_numeric(string $field, string $value, ?string $param, array $meta)
	{
		if(!preg_match("/^([0-9.]+)$/", $value))
			$this->errors[$field][] = empty($meta['error_message']) ? $meta['input_name']." should only contain numbers":$meta['error_message'];
	}
