...
class Template
{
	...
	private function parse($content)
	{
		...
		$content = preg_replace('/@for/', '<?php foreach', $content);
		$content = preg_replace('/@endfor/', '<?php endforeach ?>', $content);
		$content = preg_replace('/@if/', ' <?php if', $content);
		$content = preg_replace('/@endif/', '<?php endif ?>', $content);
		$content = preg_replace('/@else/', '<?php else: ?>', $content);
		$content = preg_replace('/\{\{(.+?)\}\}/', '<?php echo esc($1);?>', $content);
