							...
							<div class="card-body p-4">
								<form method="" enctype="multipart/form-data" class="row g-3 needs-validation" novalidate>
									<div class="col-md-6">
										...
										<input name="" type="text" class="form-control" id="bsValidation1" placeholder="" value="{{ $sales->transno }}" readonly>
									</div>

									<div class="col-md-6">
										<label for="bsValidation2" class="form-label">Product Code</label>
										<input name="" type="text" class="form-control" id="bsValidation2" placeholder="" value="{{$sales->pcode}}" readonly>
									</div>

									<div class="col-md-12">
										<label for="bsValidation3" class="form-label">Description</label>
										<input name="" type="text" class="form-control" id="bsValidation3" placeholder="" readonly value="{{$sales->description}}">
									</div>

									<div class="col-md-12">
										<label for="bsValidation3" class="form-label">Price</label>
										<input name="" type="text" class="form-control" id="bsValidation3" placeholder="" value="{{$sales->price}}" readonly>
									</div>

									<div class="col-md-12">
										<label for="bsValidation3" class="form-label">Quantity</label>
										<input name="" type="text" class="form-control" id="bsValidation3" placeholder="" value="{{$sales->qty}}" readonly>
									</div>

									<div class="col-md-12">
										<label for="bsValidation3" class="form-label">Total</label>
										<input name="" type="text" class="form-control" id="bsValidation3" placeholder="" value="{{$sales->total}}" readonly>
									</div>
									
									<div class="col-md-12 mb-3">
										<label for="bsValidation3" class="form-label">Date Purchased</label>
										<input name="" type="text" class="form-control" id="bsValidation3" placeholder="" value="{{$sales->sdate}}" readonly>
									</div>
								</form>
							</div>
						</div>
								...
								<form method="POST" class="row g-3 needs-validation" novalidate>
									<div class="col-md-12">
										...
										placeholder="" required>
											@for($admin as $row): @endphp
  											<option value="" hidden>-- Select Admin --</option>
											<option  <?=oldSelect('void_by', '{{$row->first_name}}')?> value="{{$row->first_name}}">{{$row->first_name}}</option>
										    @endfor
										</select>
									...
									<div class="col-md-12">
										...
										placeholder="" required>
											...
											<option  <?=oldSelect('cancel_request','{{$row->first_name}}')?>  value="{{$row->first_name}}">{{$row->first_name}}</option>
										    @endfor
										</select>
										<div class="invalid-feedback">
									...
									<div class="col-md-12">
										<div class="d-md-flex d-grid align-items-center gap-3">
											...
											<a href="{{BASE_URL}}/sales/daily-sales" class="btn btn-light px-4">Back</a>
										</div>
									</div>
								</form>
							</div>
